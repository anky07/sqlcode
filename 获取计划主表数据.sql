...
DECLARE @p1 VARCHAR(10) ,
      ...
      SET @pstartdate='';
      SET @penddate=''
 SELECT TOP 1 @p1 = CONVERT(VARCHAR(10), DATEADD(DAY,0,b.FPlanStartDate), 120) --取工序开始时间
            FROM    dbo.SHWorkBillEntry b
                    INNER JOIN dbo.SHWorkBill a ON a.FInterID = b.FinterID WHERE   a.FICMONO = @p3; 
            SELECT TOP 1 @p2 = CONVERT(VARCHAR(10),DATEADD(DAY,10, b.FPlanEndDate), 120) --取工序结束开始时间
            FROM      dbo.SHWorkBillEntry b
...
select @m=colid from syscolumns where object_id('tb')=id AND name=@pstartdate --查找日期列对应的其实colid id

select @n=colid from syscolumns where object_id('tb')=id AND name=@penddate   ----查找日期列对应的其实colid id
